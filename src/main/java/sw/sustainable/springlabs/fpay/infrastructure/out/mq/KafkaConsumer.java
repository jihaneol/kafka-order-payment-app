...
public class KafkaConsumer {
    ...
    private final PaymentSettlementsRepository settlementsRepository;
    ...
    public void receiveSettlements(ConsumerRecord<String, RPaymentSettlements> consumerRecord) {
        log.info("Received Settlements ::: {}", consumerRecord.value());
        RPaymentSettlements payload = consumerRecord.value();
        List<Settlements> records= payload.getSettlements();
        List<PaymentSettlements> result = records.stream().map(record -> PaymentSettlements.builder()
                .paymentKey(record.getPaymentKey())
                .method(PaymentMethod.valueOf(record.getMethod()))
                .paymentStatus(PaymentStatus.SETTLEMENTS_REQUESTED)
        ...
        settlementsRepository.bulkInsert(result);
