...
public class PaymentSettlementsRepository implements SettlementsRepository {
    private final JdbcTemplate jdbcTemplate;
    @Override
    public void bulkInsert(List<PaymentSettlements> paymentSettlements) {
        String sqlStatement = """
                INSERT INTO payment_settlements (payment_id, method, settlements_status, total_amount, pay_out_amount
                , canceled_amount, sold_date, paid_out_date)
                VALUES(?, ?, ?, ?, ?, ?, ?, ?)
                """;

        jdbcTemplate.batchUpdate(sqlStatement, paymentSettlements,  paymentSettlements.size(),
                (PreparedStatement ps, PaymentSettlements data) -> {
                    ps.setString(1, data.getPaymentKey());
                    ps.setString(2, data.getMethod().getMethodName());
                    ps.setString(3, data.getPaymentStatus().name());
