...
public class CancelService implements PaymentCancelUseCase {
    ...
    public boolean paymentCancel(CancelOrder cancelRequestMessage) throws IOException {
        ...
        PaymentLedger paymentLedgertHistory = paymentLedgerRepository.findOneByPaymentKeyDesc(paymentKey);
        if (!wantedCancelOrder.isNotOrderStatusPurchaseDecision() || !paymentLedgertHistory.isCancellableAmountGreaterThan(cancellationAmount))
            throw new IOException("OrderStatus Is Puchase Decision or Not Enough cancellation amount");

        ResponsePaymentCancel response = tossPayments.requestPaymentCancel(paymentKey, new PaymentCancel(cancelRequestMessage.getCancelReason(), cancellationAmount));
